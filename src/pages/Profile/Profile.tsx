...
const Profile: React.FC = () => {
  ...
  const convertMembers = (count: any) => {
    if (count > 999999) {
      count = `${Math.floor(count / 1000000)} млн.`;
    } else if (count > 999) {
      count = `${Math.floor(count / 1000)} тыс.`;
    }
    return count;
  };
    ...
    <>
      <div className={s['profile']}>
        <div className={s['left']}>
          ...
          <div className={s['content-block']}>
            ...
            <Swiper
              ...
              {stories.map(({ id, story, user }) => (
                <SwiperSlide key={id}>
                  <Story id={id} story={story} user={user} className={'profile'} />
                </SwiperSlide>
              ))}
              <SwiperSlide>
            ...
            <div className={s['content']}>
              <div className={s['top']}>
                <Link to="/friends" className={s['link']}>
                  ...
                  <span className={s['count']}>{friends.length}</span>
                </Link>
                <p className={s['online']}>
                  Онлайн:{' '}
                  <span>
                    {friends.filter(({ onlineType }) => onlineType !== 'pc-offline').length}
                  </span>
                </p>
              </div>
              <div className={s['friend-list']}>
                {friends.map(({ id, img, name, onlineType }, index) =>
                  index >= 4 ? null : (
                    <Link to="/friends" className={s['friend']} key={id}>
                      <HeaderAvatar
          ...
          <div className={s['content-block']}>
            ...
            <div className={s['content']}>
              ...
              <div className={s['group-list']}>
                {groups.map(({ id, img, name, members }, index) =>
                  index >= 3 ? null : (
                    <Link to="/groups" className={s['group']} key={id}>
                      <HeaderAvatar
                        className={'content-block'}
                        img={img}
                        title={''}
                      ...
                      <div className={s['group-info']}>
                        <span className={s['group-name']}>{name}</span>
                        <div className={s['members']}>
                          ...
                          <span>{convertMembers(members)}</span>
                        </div>
                      </div>
                    </Link>
                  ),
            ...
            <div className={s['content']}>
              <div className={s['top']}>
                <Link to="/groups" className={s['link']}>
                  ...
                  <span className={s['count']}>{music.length}</span>
                </Link>
              </div>
              <div className={s['group-list']}>
                {music.map(({ id, img, name, author }, index) =>
                  index >= 3 ? null : (
                    <Link to="/music" className={s['group']} key={id}>
                      <div className={s['music-img']}>
                        <img src={ava} alt="music" />
                      </div>
                      <div className={s['group-info']}>
                        <span className={s['group-name']}>{name}</span>
                        <span className={s['members']}>{author}</span>
                      </div>
                    </Link>
                  ),
                )}
          ...
          <div className={s['content-block']}>
            ...
            <div className={s['content']}>
              ...
              <div className={s['group-list']}>
                {videos.map(({ id, video, name, author, views }, index) =>
                  index >= 3 ? null : (
        ...
        <div className={s['right']}>
          ...
          <div className={s['collection-block']}>
