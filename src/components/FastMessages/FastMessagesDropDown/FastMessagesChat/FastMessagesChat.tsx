...
const FastMessagesChat: React.FC = () => {
    ...
    const chats = [
        ...
        { id: 2, userId: 10, messageText: 'я тебе ответил очень конструктивно дааааа кмон', messageTime: '12:40', img: ebalo2 },
        { id: 4, userId: parseInt(dialogId), messageText: 'И короче ещё одно сообщение лол', messageTime: '12:40', img: ebalo2 },
        { id: 5, userId: parseInt(dialogId), messageText: 'jdsahkjdhsajdshaj', messageTime: '12:40', img: ebalo2 },
        { id: 6, userId: parseInt(dialogId), messageText: 'jdsahkjdhsajdshaj', messageTime: '12:40', img: ebalo2 },
        ...
        <>
            <div className={s['wrapper-top']}>
                <Link className={s['user-link']} to='/'>
                    <HeaderAvatar hasDelete={false} className='fm-chat-image' img={ebalo} title={'image'} indicatorClass='fm-indicator' />
                    <span className={s['user-name']} title={dialogId}>{dialogId}</span>
                </Link>
            ...
            <div className={s['wrapper-content']}>
                {
                    chats.map(({ id, userId, messageText, messageTime, img }, index, chats) => (<div key={id} className={s['message-wrapper']}>
                        {
                            userId !== chats[index + 1]?.userId && <HeaderAvatar hasDelete={false} className='message-avatar' img={img} title={'image'} />
                        }
                        <div className={`${userId === parseInt(dialogId) ? s['other-message'] : s['my-message']} ${s['message-text']} ${userId === chats[index + 1]?.userId && s['left-margin']}`}>
                            {messageText}
